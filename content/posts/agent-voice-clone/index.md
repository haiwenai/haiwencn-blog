...
段永平的《大道：投资问答录》，509页，33万字。我断断续续读了两个月，卡在第二章。

不是不想读，是生活不给整块时间。拿起来读三页，女儿喊你，放下。第二天翻开，忘了昨天在哪，从上一节重来。两个月，进度条几乎没动。

那天下午去接女儿放学，开车走到半路突然想到一件事：能不能用一个我喜欢的声音，把这本书读给我听？

不是去喜马拉雅找一个播音员录好的版本——这本书根本没有。是**我指定的声音**，读**我指定的内容**。开车听，做饭听，带孩子去公园时挂个耳机听。阅读的瓶颈不是眼睛，是场景。耳朵能到达眼睛去不了的地方。
...
- **audiobook-generator**：依赖 SenseAudio 的 API key，需要单独申请，而且文档没说清楚定价和限制。
- **audiobooklm**：它的 `synthesize_tts` 函数有 **200 字的硬限制**——一本33万字的书，按200字一段切？那不是有声书，是碎片轰炸。

两个都不能干净地完成"PDF → 完整逐字朗读 → 按章节输出"这件事。
...
- 上传几分钟录音，平台返回一个 voice_id（我的是 `b04f960f`）
- 给它任何文本，返回**你自己的声音**
- 每 500 字一次 API 调用，每次约 50-80 秒返回
- 全书33万字 ≈ 633 次 API 调用 ≈ **10-12 小时**
- API key 存在本地 `~/.noiz_api_key`，调用走 Python requests（curl 对长 payload 不稳定）

**最终决定两个都留。** Edge 做快速调试和试听，Noiz 做正式版。skill 里加了 `--engine` 参数，一行命令切换：

```bash
# 快速试听（Edge 女声，几分钟出结果）
python3 pdf_to_audiobook.py --pdf book.pdf --engine edge

# 正式版（克隆声音，跑一晚上）
python3 pdf_to_audiobook.py --pdf book.pdf --engine noiz
```
...
**修复方案：** 在 `clean_text_for_tts` 函数里加判断——上一行如果不以句号/问号/感叹号/引号结尾，说明这个换行是排版导致的，合并；否则保留为真正的段落分隔。

这个 bug 如果不是自己真的去听了生成的音频，可能永远不会发现。

### 坑二：页眉页脚混入正文

每页顶部的书名、页码会被提取成正文的一部分。清洗阶段统一去除。

### 坑三：版权页和目录页

前 11 页是出版信息，12-20 页是目录，22 页才是"第一章 投资大道"。如果不跳过，语音引擎会认真朗读出版社地址和 ISBN 号——没人想听这个。

**修复方案：** 加了 `--start-page` 和 `--end-page` 参数，跳过非正文页。这本书用 `--start-page 22`。

## 五、章节切分：最折腾的部分

33万字不能一口气扔给语音引擎。要按章节切分，每章生成一个独立的 MP3。
...
初版用的 pydub 合并音频。但 Python 3.14 移除了 `audioop` 模块，pydub 直接报错。改成 ffmpeg concat 后再没出过问题。**这是一个已被替代的决策：pydub merge 方案已废弃，现行方案是 ffmpeg merge。**
...
<!-- TODO: 配截图——Obsidian 界面，音频播放器+原文并排 -->
...
`pdf-to-audiobook` 完整代码在我的 OpenClaw skill 目录下：

```
~/.openclaw/skills/pdf-to-audiobook/
├── SKILL.md                    ← 使用文档
└── scripts/
    └── pdf_to_audiobook.py     ← 核心脚本
```
